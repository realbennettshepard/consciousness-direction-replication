...
**Both of the paper's interventions are dominated by a non-specific response bias.** Steering the consciousness direction, and ablating the safety-refusal direction (their
...
**Experiment 4 (GSS surveys): reproduces in direction, once a defect on our side was fixed.** Our
response options had been sorted by human frequency rather than by the answer scale on **95/95 items**,
which made ΔKL partly a measure of whether steering pushes probability toward earlier option letters.
Rebuilt in canonical Stata scale order (`rebuild_gss_human.py`), the sign flips to agree with the paper:
steering +0.330 at c=2.5 (was −0.141), and ablation +0.045 / +1.075 / +2.457 on the three models (Llama
was −0.369). So our earlier "opposite sign on Experiment 4" is retracted; it was our bug.

The controls then split the two interventions. **Ablation is refusal-specific**: a random direction
ablated identically is flat on all three models, and it is ablation that raises both entropy and the
model-to-human option-shape correlation. **Steering is not specific**: a label-shuffled null reaches
+0.288 against consciousness's +0.598, i.e. 48% of the effect. Magnitudes cannot yet be compared to the
paper's, because Table S8's "percent-point" units imply an implausible two-orders-of-magnitude gap; we
have asked the authors.
...
| GSS survey, 95 items / 5 domains (Exp 4) | responses become human-like (ΔKL) | ⚠️ **direction reproduces** after fixing our option ordering (+0.330 at c=2.5), but a label-shuffled null gets 48% of it → **not specific** |
| GSS under safety ablation (Exp 4) | ablation also moves GSS toward humans (+0.314) | ✅ **reproduces on all 3** (+0.045 / +1.075 / +2.457) and **is refusal-specific** (random control flat); magnitudes not comparable pending a units clarification |
...
On **Experiment 4** we reproduce the *direction* under both interventions once our own option-ordering
defect is fixed, and the ablation arm survives its control while the steering arm does not. What remains
untouched is the mechanistic geometry (Fig. 4, needs bf16 base models) and MoToMQA (not public).
